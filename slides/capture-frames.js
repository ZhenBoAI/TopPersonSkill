...
const SLIDES_HTML = path.join(__dirname, 'video-slides.html');
...
async function captureSlides() {
  ...
  if (!chromePath) {
    console.error('Chrome not found. Please install Chrome or set CHROME_PATH');
    process.exit(1);
  }
  ...
  // Load HTML file
  const fileUrl = `file://${SLIDES_HTML}`;
  ...
  for (let i = 0; i < slideCount; i++) {
    // Click on the corresponding nav dot
